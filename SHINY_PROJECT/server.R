...
###
default_by_purpose =  ggplot(german_credit_t, aes(Purpose, fill = factor(Creditability))) +
  ...
  scale_fill_hue(labels=c("Not credible", "Credible")) + theme(text=element_text(size=14))
###
default_by_occupation =  ggplot(german_credit_t, aes(Occupation, fill = factor(Creditability))) +
  geom_bar(position = "dodge") + guides(fill=guide_legend(title="Creditability")) +
  ggtitle("Credit default proportion grouped by occupation") + theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_fill_hue(labels=c("Not credible", "Credible")) + theme(text=element_text(size=14))
###
default_by_fam_status =  ggplot(german_credit_t, aes(Sex...Marital.Status, fill = factor(Creditability))) +
  geom_bar(position = "dodge") + guides(fill=guide_legend(title="Creditability")) +
...
server <- function(input, output,session) {
  ...
  output$var_scatterplot <- renderPlot({
    ...
    filtered_data =  german_credit_01 %>%
      ...
      ggplot(filtered_data, aes(y= Credit.Amount, x=Age..years.)) + geom_point(aes(colour = factor(Creditability)),  size = 3, position = position_jitterdodge()) +
